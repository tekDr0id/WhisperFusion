...
=======
## CRITICAL FIX: Install PyAV with prebuilt wheels (v12.0.0+)
...
## Now install all application dependencies from requirements.txt
echo "Installing application dependencies from requirements.txt..."
uv pip install --system -r requirements.txt

## Install faster-whisper with preference for newer version that supports av 12.x
echo "Installing faster-whisper..."
uv pip install --system 'faster-whisper>=1.0.0' || uv pip install --system --no-deps faster-whisper==0.9.0

## Verify critical dependencies are installed
echo "Verifying critical dependencies..."
python3 -c "import webdataset; print('✓ webdataset module verified')" || \
    { echo "ERROR: webdataset failed to import"; exit 1; }
...
## Download models using hf download (new command replacing deprecated huggingface-cli)
echo "Downloading models from Hugging Face..."
hf download collabora/whisperspeech t2s-small-en+pl.model s2a-q4-tiny-en+pl.model
hf download charactr/vocos-encodec-24khz
